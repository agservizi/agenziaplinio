...
function ap_create_order(int $userId, array $items, array $payload): int
{
    ...
    try {
        ...
        ap_send_order_notification($orderId);
...
function ap_process_payment(array $payload): void
{
    $pdo = ap_db();
    $orderId = $payload['order_id'] ?? null;
    if (!$orderId) {
        throw new InvalidArgumentException('Order ID is required for payment processing');
    }
...
function ap_update_shipping(int $orderId, array $data): void
{
    ...
    $newStatus = $data['shipping_status'] ?? ($current['shipping_status'] ?? 'preparing');
    $oldStatus = $current['shipping_status'] ?? null;
    ...
    // Send shipping notification if status changed to shipped/in_transit
    if ($oldStatus !== $newStatus && in_array($newStatus, ['in_transit', 'shipped'])) {
        ap_send_shipping_notification($orderId);
    }
}
